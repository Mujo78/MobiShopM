...
export default function Overview(){
    ...
    return (
        <Container className={`d-flex flex-column ${!isDesktop && `p-0`}`}>
            <h3>Personal info</h3>
            <Container className={`w-100 mt-3 d-flex ${isMobile ? `p-0 justify-content-start flex-wrap` : `justify-content-around` }`}>
                ...
                <Container className={`${isMobile ? `w-100` : `w-50 me-5`}`}>
                    <CustomListGroup variant="flush">
                        ...
                        <>
                            ...
                            <label>Email</label>
                            <ListGroup.Item className="mb-3">{infoPersonState.email}</ListGroup.Item>
                        </> :
                        <>
                            <label>Last name</label>
                            <ListGroup.Item className="mb-3">{infoPersonState.last_name}</ListGroup.Item>
