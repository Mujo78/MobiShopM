...
export default function DeleteMobile() {
  ...
  const getMobilesForBrand = (id) => {
    axios
      .get(`http://localhost:3001/api/brand-mobiles/${id}`)
      .then((response) => {
        setMobileData((n) => ({
          ...n,
          [id]: response.data,
  ...
  const deleteMobile = (brandID, id, name) => {
    axios
      .delete(`http://localhost:3001/delete-mob/${name}`, {
        headers: {
  ...
  const deleteBrand = (id, name) => {
    axios
      .delete(`http://localhost:3001/delete-brand/${name}`, {
        headers: {
          accessToken: `Bearer ${localStorage.getItem("accessToken")}`,
        },
      })
  ...
  return (
    <Container className="d-flex flex-column flex-grow-1">
      ...
      >
        {numErrorsBrands <= 0 ? (
          brandsData.slice(indexOfFirstRecord, indexOfLastRecord).map((n) => (
            <Accordion key={n.id} activeKey={activeKeys} className="me-2 mb-2">
              <Accordion.Item eventKey="0">
                <AccordionButton onClick={() => getMobilesForBrand(n.id)}>
                  {n.name}
                </AccordionButton>
                <Accordion.Body>
                  <ListGroup
                  ...
                  >
                    <ListGroup.Item>
                      <Button
                        onClick={() => deleteBrand(n.id, n.name)}
                        variant="danger"
                        style={{ border: "none" }}
                      >
                        Delete brand
                    ...
                    {numErrorsMobile <= 0 ? (
                      mobileData[n.id] &&
                      mobileData[n.id].map((m) => (
                        <ListGroup.Item key={m.id}>
                          <Container className="d-flex p-0 flex-column">
                            <h5>{m.mobile_name}</h5>
                            <p>
                              {m.internal}GB {m.ram}GB RAM
                            </p>
                          </Container>
                          <Button
                            onClick={() =>
                              deleteMobile(n.id, m.id, m.mobile_name)
                            }
                            style={{
          ...
          <Alert variant="danger">{errorsBrands}</Alert>
        )}
      </Container>
      {brandsData.length > 0 && (
